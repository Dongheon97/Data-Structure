...
public class AppController {
	...
	private void elementAt() {
		...
		if(order < 0 || order >= this.list().size()) {
			...
			AppView.outputLine("! 입력된 순서 ["+order+"]의 학생의 점수는 ("+ student.score()
								+")점 입니다.");
		}
	}
		...
		if(this.list().isFull()) {
			...
			if(order<0 || order>this.list().size()) {
				AppView.outputLine("! 입력된 순서 ["+order+"]가 정상 범위 [0.."
									+this.list().size()+"]에 있지 않습니다.");
			}
			else {
				int score = this.inputScore();
				if(this.list().addTo(new Student(score), order)) {
					AppView.outputLine("! 입력된 순서 [" +order+"]에 입력된 점수 ("+score
										+")의 학생을 삽입하는 작업을 성공하였습니다.");
				}
				else {
					AppView.outputLine("! 입력된 순서 [" +order+"]에 입력된 점수 ("+score
										+")의 학생을 삽입하는 작업을 실패하였습니다.");
				}
			}
		}
	}
	...
	private void addToFirst() {
		...
		if(this.list().isFull()) {
			...
			if(this.list().addToFirst(new Student(score))) {
				AppView.outputLine("! 입력된 점수 [" + score +"]의 학생을 [맨 앞]에 삽입하는 작업을 "
									+ "성공하였습니다.");
			}
			else {
				AppView.outputLine("! 입력된 점수 [" + score +"]의 학생을 [맨 앞]에 삽입하는 "
									+ "작업을 실패하였습니다.");
			}
		}
	}
	...
	private void addToLast() {
		...
		if(this.list().isFull()) {
			...
			if(this.list().addToLast(new Student(score))) {
				AppView.outputLine("! 입력된 점수 [" + score +"]의 학생을 [맨 뒤]에 삽입하는 "
									+ "작업을 성공하였습니다.");
			}
			else {
				AppView.outputLine("! 입력된 점수 [" + score +"]의 학생을 [맨 뒤]에 삽입하는 "
	...
	private void add() {
		...
		if(this.list().isFull()) {
			...
			if(this.list().add(new Student(score))) {
				AppView.outputLine("! 입력된 점수 [" + score +"]의 학생을 [임의의 순서]에 삽입하는 "
									+ "작업을 성공하였습니다.");
			}
			else {
				AppView.outputLine("! 입력된 점수 [" + score +"]의 학생을 [임의의 순서]에 삽입하는 "
		...
		if(this.list().isEmpty()) {
			...
			if(order<0 || order>=this.list().size()) {
				...
				if(student != null) {
					...
					AppView.outputLine("! 입력된 순서 ["+ order + "]에서 학생을 삭제하는 작업을 "
										+ "실패하였습니다.");
				}
			}
		}
		...
		if(this.list().isEmpty()) {
			...
			if(order < 0 || order>this.list().size()) {
				...
				if(this.list().replaceAt(new Student(score), order)) {
					AppView.outputLine("! 주어진 순서 ["+order+"]의 학생의 점수가 ("+score
										+")로 바뀌었습니다.");
				}
				else {
	...
	private void clear() {
		if (this.list().isEmpty()) {
			...
			this.list().clear();
			AppView.outputLine("! 초기화 작업을 완료했습니다.");
		}
	}
